...
public:
    ...
    inline S& arrowType(Qt::ArrowType type)                     { t->setArrowType(type); return self(); }
    inline S& popupMode(QToolButton::ToolButtonPopupMode mode)  { t->setPopupMode(mode); return self(); }
    inline S& autoRaise(bool b)                                 { t->setAutoRaise(b);    return self(); }
